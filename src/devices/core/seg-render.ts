 ...
 */
export const drawSegWebgpu: NonNullable<DevicePlugin['drawWebgpu']> = (instance, renderPass, globalUniformBuffer, skipEffects) => {
  ...
  if (instance.rollerInstances && instance.segEnhancedPipeline) {
    ...
    const rings = instance.visualizer.segLayout?.rings;
    const outerR = ((rings && rings[rings.length - 1]?.orbitRadiusM)
      ?? 5.5) * (instance.visualizer.segLayout?.worldScale ?? 1);
    const cam = instance.visualizer.camera?.camera;
    const inside = cam?.position
      ? isCameraInsideSegRing(cam.position, segPos as number[], outerR)
      : false;
