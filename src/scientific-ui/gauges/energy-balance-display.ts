...
export interface EnergyFlows {
  ...
  panelElectrical: number;
  batteryIn: number;
  roundTripEff: number;
}

export type EnergyFlowsUpdate = Partial<EnergyFlows>;

export class EnergyBalanceDisplay {
  container: HTMLElement;
  flows: EnergyFlows;
  efficiency: typeof LED_SOLAR_CONSTANTS.EFFICIENCY;
   ...
   * @param flows - Energy flow data
   */
  updateFlows(flows: EnergyFlowsUpdate): void {
  ...
  updateDisplay(): void {
    ...
    Object.entries(widths).forEach(([selector, width]) => {
      const el = this.container.querySelector(selector);
      if (el) el.setAttribute('stroke-width', String(width));
    });
