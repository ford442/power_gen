...
| `transformer` | 10 | 8 (`SIM_MODE_TRANSFORMER`) | `transformerVp` (V), `transformerVs` (V), `transformerIpA` (A), `transformerIsA` (A), `transformerK`, `transformerFluxN` (Wb) | WASM coupled-inductor ODE (?wasmPhysics=1); JS phasor fallback |
