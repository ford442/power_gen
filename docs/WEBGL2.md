...
| Lab field coupling | Yes — `FieldNetwork` writes the coupled B into the plant before either backend steps, so `?fieldCoupling=1` behaves identically here and on WebGPU (ADR-0011) |
